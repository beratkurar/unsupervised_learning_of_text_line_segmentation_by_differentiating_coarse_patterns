...
output_folder='crop_cb55_xml/'
polygon_labels_dir= 'crop_cb55_polygon_labels/'
...
for original_path in original_path_list:
    ...
    Created.text = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    Metadata.append(Created)
    LastChange = ET.SubElement(Metadata, 'LastChange')
    LastChange.text = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    myfile = open('crop_cb55_xml/' + original_page_name[:-4] + '.xml', "w")
