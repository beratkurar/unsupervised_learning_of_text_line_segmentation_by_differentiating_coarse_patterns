...
def pca(outer_size, inner_size, test_set, results_folder_name):
    ...
    #vis_model = Model(inputs = vis_model1.input, outputs = vis_model1.get_layer('conv_4').output)
    ...
    for imgp in os.listdir(test_set): 
        ...
        for i in range(bgr_rows):
            for j in range(bgr_cols):
                pixel_value=bgr[i,j,:]
                result[i*inner_size:i*inner_size+inner_size,j*inner_size:j*inner_size+inner_size,:]=pixel_value

        #upsampled_nearest = np.asarray(result, dtype=np.uint8)
        upsampled_nearest = cv2.resize(bgr, (cols, rows), cv2.INTER_NEAREST)
        blended_upsampled_nearest=cv2.addWeighted(upsampled_nearest, 0.9, cv2.merge((page, page, page)), 0.1, 0)
        
        upsampled_cubic=cv2.resize(bgr, (cols, rows), cv2.INTER_LINEAR)
...
pca(350, 20, test_set, 'temp_results')
