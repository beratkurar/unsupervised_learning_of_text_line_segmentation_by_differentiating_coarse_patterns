function [ result] =moc_manual_blobs_PostProcessByMRF(L,num,LabeledLines, numLines, charRange, options )
    CCsparseNs = computeNsSystem( L,num, options );
    Dc  = computeLinesDC(LabeledLines,numLines,L,num, charRange(2));
    [Labels] = moc_manual_blobs_LineExtraction_GC_MRFminimization(numLines, num, CCsparseNs,Dc);
