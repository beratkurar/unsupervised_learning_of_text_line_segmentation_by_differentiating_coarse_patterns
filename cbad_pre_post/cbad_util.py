...
def save_and_resize(img: np.array, filename: str, size=None) -> None:
    if size is not None:
        resized = cv2.resize(img, fx=size,fy=size)
...
def draw_lines_fn(xml_filename: str, output_dir: str):
    ...
    cv2.imwrite(os.path.join(output_dir, '{}.jpg'.format(basename)), drawing_img)


cbad_set_generator('train', 'label', 0.5, 4,False, 20)